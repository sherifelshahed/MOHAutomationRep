	...
	public void transfer(WebDriver driver, String Patient_ID) {
		...
		try {
			...
			if (driver.getTitle().contains("Intra ward Patient Transfer")) {
				...
				// Select ReasonForTransfer = new
				// Select(driver.findElement(By.name("transfer_type")));
				// ReasonForTransfer.selectByVisibleText("Operation");
			...
			for (String window1 : winds) {
				driver.switchTo().window(window1);
				System.out.println(driver.getTitle());

				if (driver.getTitle() != "Bed Availability Chart") {
					driver.close();
				}
			}
			...
			if (driver.switchTo().alert().getText().contains("APP-SM0071 Operation Completed Successfully ....")) {
